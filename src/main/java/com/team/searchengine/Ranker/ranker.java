...
public class ranker {
    ...
    public void score(ArrayList<wordResult> r,ArrayList<String> words) {
        ...
        this.romee=r;
//increase the rank of the links based on the number of times they appear in the page
...
/////to add rank based on the tf*idf
        ...
        for (int j = 0; j < r.size(); j++) {
            for (int i = 0; i < r.get(j).Links.size(); i++) {
                if (links.containsKey(r.get(j).Links.get(i)))
                    links.put(r.get(j).Links.get(i), links.get(r.get(j).Links.get(i)) + 15 * r.get(j).TF.get(i) * r.get(j).idf);
                else links.put(r.get(j).Links.get(i), 15 * r.get(j).TF.get(i) * r.get(j).idf);
            }
        }
...
for (int j = 0; j < r.size(); j++) {
    for (int i = 0; i < r.get(j).Titles.size(); i++) {
        ...
        if (Str.toLowerCase().contains( words.get(j).toLowerCase() )) {
            ...
            else links.put(r.get(j).Links.get(i), 5 * titleweight);
        }
    }
}
...
//////to add the rank of pagerank
        for (int j = 0; j < r.size(); j++) {
            for (int i = 0; i < r.get(j).Links.size(); i++) {
                ...
                else links.put(r.get(j).Links.get(i), 20.0 * r.get(j).ranks.get(i));
            }
        }
